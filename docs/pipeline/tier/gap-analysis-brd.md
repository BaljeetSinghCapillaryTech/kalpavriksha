...
| G-2 | **PartnerProgramSlab not addressed.** PRD has no acceptance criteria for what happens to partner program slab mappings when a program slab is stopped/deleted. | PartnerProgramSlab.java, PePartnerProgramSlabDao.java | HIGH -- could break partner program integrations. | Add validation: block stop/delete if PartnerProgramSlabs reference this slab, or cascade. |
