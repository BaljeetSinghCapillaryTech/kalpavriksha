...
| D-32 | `LoyaltyExtendedFieldRepository` uses JPQL `@Query` with dynamic scope/isActive filter for list endpoint. Spring Data derived queries used for findByPkIdAndPkOrgId and existsByPkOrgIdAndProgramIdAndScopeAndName uniqueness check. | Follows `HistoricalPointsDao.java` pattern — `@Query` + `@Param` for multi-condition JPQL; derived queries where derivable | Designer | C7 |
| D-33 | `ExtendedFieldValidationException` is a RuntimeException (unchecked) thrown from `ExtendedFieldValidator`. Caught by `SubscriptionErrorAdvice` handler → HTTP 400. Not propagated via Thrift — validation runs in V3 before any Thrift call for subscription write path. | Keeps validation concern in V3; EMF Thrift call is read-only (fetches configs) for this path | Designer | C7 |
