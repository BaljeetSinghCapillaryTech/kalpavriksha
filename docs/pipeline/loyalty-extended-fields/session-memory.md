...
| D-25 | `name` is mutable via PUT. Since D-28 moves validation to id-based lookup (not name-based), renaming no longer orphans MongoDB documents. Name is display-only. | id stored in MongoDB, not name | C-28 answer | C7 |
...
| D-27 | `SubscriptionProgram.ExtendedField.type` field is **deleted entirely** (not renamed). Scope is always `SUBSCRIPTION_META` for subscription programs — backend validates. `SubscriptionProgram.ExtendedField` model: `{id: Long, value: String}` only. EF-US-07 updated: delete `type` field instead of rename type→scope. No MongoDB migration needed. | scope implicit, always SUBSCRIPTION_META; EF-US-07 simplified | C-2/3 answer | C7 |
| D-28 | `SubscriptionProgram.ExtendedField` stores `{id: Long, value: String}` where `id` is FK to `loyalty_extended_fields.id`. All validation (R-02) is by id, not by name. Name is display-only — mutable without risk. | eliminates C-28 referential integrity risk | C-28 answer | C7 |
...
| A-03 | `@Field("key")` annotation on `SubscriptionProgram.ExtendedField.id: Long` — keeps MongoDB document key as `"key"`, avoids any migration for existing documents. Old documents deserialize `id=null` gracefully (no EF validation applied). | Evidence: OQ-1 — confirmed no `@Field` on existing `key: String` field; new `id: Long` replaces it with `@Field("key")` mapping | Architect | C6 |
