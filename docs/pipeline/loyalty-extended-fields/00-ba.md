...
**EF-US-07** — Remove `ExtendedFieldType` and simplify `SubscriptionProgram.ExtendedField`
> As a developer, I want the wrong `ExtendedFieldType` enum and `type` field removed, and `SubscriptionProgram.ExtendedField` simplified to `{id: Long, value: String}`.

Acceptance Criteria:
- `ExtendedFieldType` enum (`CUSTOMER_EXTENDED_FIELD`, `TXN_EXTENDED_FIELD`) is deleted — **D-27**
- `SubscriptionProgram.ExtendedField.type` field is deleted (not renamed) — scope always SUBSCRIPTION_META, validated server-side
- `SubscriptionProgram.ExtendedField.key` (String) replaced by `id` (Long — FK to `loyalty_extended_fields.id`) — **D-28**
- Tests BT-EF-01 to BT-EF-06 updated to use `id`-based model; remove all `ExtendedFieldType` references
- No other callers of `ExtendedFieldType` broken (3 confirmed usages only — all in scope of this story)
- No MongoDB migration needed — existing documents retain old shape; new code reads `id` field; Architect to confirm `@Field` annotation strategy for `key`→`id` rename in existing documents
