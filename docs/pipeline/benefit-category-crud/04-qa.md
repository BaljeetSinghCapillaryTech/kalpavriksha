...
### QA-004 — Create category: name collision on active row → 409
**[P0] [IT] [ADR-009] [AC-BC02] [D-28]**
...
### QA-032 — Update: clear all slabs (empty slabIds permitted on Update)
**[P1] [IT] [ADR-003] [D-35]**

**Given** active category id=42 with slabs=[1,2] in org 100.
**When** `PUT /v3/benefitCategories/42` with `{name:"VIP", slabIds:[]}`.
**Then**
- Response: `200 OK`; `data.slabIds = []`.
- All existing mappings soft-deleted.

> **Note**: Empty `slabIds` is permitted on Update (`@NotNull` but no `@Size(min=1)`) per Designer `BenefitCategoryUpdateRequest`. SDET must confirm this matches the `@Size` constraint choice — see ASSUMPTIONS MADE §A.
...
| `BC_BAD_ACTIVE_FILTER` (A7-07) | 400 | — | ⚠ Not explicitly tested — see QUESTIONS FOR USER |
...
| # | Question | Confidence | Impact if wrong |
|---|----------|------------|-----------------|
| Q8-01 | `BenefitCategoryUpdateRequest` has `@NotNull` but NO `@Size(min=1)` on `slabIds` — meaning an empty list `[]` is accepted and clears all mappings. Is this the intended behaviour (clear all mappings via PUT)? Or should empty slabIds on Update also be rejected? **QA-032 documents this as acceptable; if you want to reject empty on Update, add `@Size(min=1)` and a `BC_SLAB_IDS_REQUIRED` test case.** | C5 | If wrong, QA-032 and QA-033 need revision; an additional test for "Update with empty slabIds → 400" must be added. |
| Q8-02 | Is name uniqueness check **case-insensitive** (e.g., "VIP Perks" and "vip perks" should conflict) or **case-sensitive**? Assumption A7-06 says case-sensitive (no `LOWER()` applied). If product requires case-insensitive, additional scenarios are needed (QA-004 currently only tests exact-case match). | C5 | If case-insensitive: add a test "POST with same name in different case → 409"; update QA-004 to cover this. |
| Q8-03 | What is the exact `BC_BAD_ACTIVE_FILTER` code and HTTP status for `?isActive=foo` (unrecognised value)? Assumption A7-07 says HTTP 400; code name is `BC_BAD_ACTIVE_FILTER`. SDET needs to confirm this code string to assert in QA. | C4 | SDET cannot write the assertion if the code is unknown. Need confirmation before Phase 9. |
...
|---|------------|------------|-------------|
| A8-01 | `BenefitCategoryUpdateRequest.slabIds` accepts empty list (`[]`) on Update — clears all mappings. Derived from `@NotNull` (not `@Size(min=1)`) on Designer §F.8 `BenefitCategoryUpdateRequest`. | C5 | QA-032 documents this as valid. If wrong, QA-032 becomes invalid and a "400" test replaces it. |
| A8-02 | Name uniqueness check is case-sensitive (no `LOWER()` wrapper). Derived from Assumption A7-06 in Designer doc. | C5 | QA-004/QA-036 test exact-case match only. |
...
- [risk] `BC_BAD_ACTIVE_FILTER` (400 on invalid `isActive` query-param) is undefined in ADR-009 but implied by A7-07 — needs explicit code string confirmation before SDET Phase 9. _(QA)_ — Status: open
- [risk] `NotFoundException → HTTP 200` platform quirk (OQ-45) creates test fragility — if the quirk is fixed in a concurrent ticket, all "not found" scenario assertions will need updating across 10+ scenarios. _(QA)_ — Status: open
- [risk] `BenefitCategoryUpdateRequest.slabIds` — `@NotNull` without `@Size(min=1)` allows clearing all mappings via PUT with `slabIds:[]`. This may be unintended if product wants "at least one slab at all times" invariant on active categories. _(QA)_ — Status: open
...
- [ ] Q8-03: Confirm `BC_BAD_ACTIVE_FILTER` code string for `?isActive=foo` validation error. _(QA)_
