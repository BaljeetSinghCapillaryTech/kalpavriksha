...
| `customFields` on `SubscriptionProgram` is referenced in ONLY 5 places total: field declaration (line 133), class definition (lines 291-298), CustomFieldRef class (301-304), and 4 facade pass-throughs (lines 85, 271, 325, 367). `SubscriptionPublishService` does NOT reference it. Zero test files reference it. | LSP references + grep C7 _(Architect EF Addendum)_ |
| `CustomFieldMapping.Type` enum in emf-parent (CustomFieldMapping.java:14): `BE_ATTRIBUTE, USER_INPUT, TXN_EXT_FIELDS, LINE_ITEM_EXT_FIELDS, CUSTOMER_EXTENDED_FIELDS, POINTS, CUSTOMER_CUSTOM_FIELDS, TXN_CUSTOM_FIELDS`. API-level names `TXN_EXTENDED_FIELD` / `CUSTOMER_EXTENDED_FIELD` map to `TXN_EXT_FIELDS` / `CUSTOMER_EXTENDED_FIELDS` at evaluation time. | CustomFieldMapping.java:14 C7 _(Architect EF Addendum)_ |
| `CustomFieldMappingHelper.getRawValue()` uses `field.getMappedToValue()` (= key) to look up value from payload at evaluation time. Values are strings; numeric coercion done by `parseExtendedFieldDataForNumericValue()`. No type coercion needed in intouch-api-v3. | CustomFieldMappingHelper.java:122-163 C7 _(Architect EF Addendum)_ |
...
| KD-62 | Extended Fields SUPERSEDES Custom Fields (OQ-20, ADR-19). `SubscriptionProgram.CustomFields` and `CustomFieldRef` inner classes are DELETED. New field: `List<ExtendedField> extendedFields` (embedded in MongoDB doc). New inner class: `ExtendedField{type: ExtendedFieldType, key: String, value: String}`. New enum: `ExtendedFieldType{CUSTOMER_EXTENDED_FIELD, TXN_EXTENDED_FIELD}`. MongoDB storage format: `[{type, key, value}, ...]`. Values stored as strings — type identification at evaluation time via emf-parent `CustomFieldMappingHelper.getRawValue()`. No MySQL, no Thrift, no new API endpoints. ADR-19 accepted. _(Architect, 2026-04-20)_ | User decision + LSP-verified blast radius (2 files modified, 1 enum added) | Architect EF Addendum | 2026-04-20 |
| KD-63 | Extended Fields CRUD: No dedicated `/v3/extendedfields` APIs. EF data accepted/returned directly via existing Subscription POST (create) and PUT (update) endpoints. Keys are pre-populated by UI — no server-side key catalog validation in this scope. Multiple `ExtendedField` entries of the same or different type allowed per subscription. _(Architect, 2026-04-20)_ | User decision | Architect EF Addendum | 2026-04-20 |
| KD-64 | `ExtendedField.value` is nullable — a field may be configured with no static value (resolved from payload at evaluation time). `ExtendedField.type` and `ExtendedField.key` are `@NotNull`/`@NotBlank`. R-33 (update path) mirrors `reminders` null-guard pattern: only overwrites when caller provides the field. _(Designer, 2026-04-20)_ | Designer Rework 5 | Designer Rework 5 | 2026-04-20 |
| KD-65 | Rework 5 implementation: 1 enum created (`ExtendedFieldType`), 2 files modified (`SubscriptionProgram.java`, `SubscriptionFacade.java`). All 111 existing subscription tests GREEN after changes. No new dependencies added. _(Developer, 2026-04-20)_ | Developer Rework 5 | Developer Rework 5 | 2026-04-20 |
...
| OQ-20 | Custom fields storage type: `metaCustomFields`, `linkCustomFields`, `delinkCustomFields` — `Map<String, Object>` or `Map<String, String>`? | Architect Phase 6 | **SUPERSEDED (KD-62, ADR-19)** — CustomFields entirely replaced by `List<ExtendedField>`. OQ-20 Designer answer (List<CustomFieldRef>) is now obsolete. _(Architect EF Addendum, 2026-04-20)_ |
| EF-OQ-01 | At evaluation time, what is the mapping from `CUSTOMER_EXTENDED_FIELD` / `TXN_EXTENDED_FIELD` → `CustomFieldMapping.Type.CUSTOMER_EXTENDED_FIELDS` / `TXN_EXT_FIELDS`? Does emf-parent receive EF list via Thrift or read MongoDB directly? | Architect EF Addendum | **DEFERRED** — evaluation phase out of scope for E3 |
| EF-OQ-02 | Max cardinality of `extendedFields` list per subscription? No `@Size` cap specified. | Designer Rework 5 | **OPEN — LOW — Developer can add `@Size(max=N)` if product confirms a cap** |
