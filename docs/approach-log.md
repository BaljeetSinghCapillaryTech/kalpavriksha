...
| # | Question | Decision | Rationale |
|---|----------|----------|-----------|
| D-27 | Which annotation/injection pattern for new subscription types? | **Follow existing UnifiedPromotion patterns exactly**: @Component (not @Service) for facade/validators/publisher, @Autowired field injection (not constructor), @RestController with constructor injection for controller. | Step 0 codebase discovery: UnifiedPromotionFacade uses @Component + @Autowired field injection. UnifiedPromotionController uses constructor injection. Consistency via Clone-and-Adapt (KD-18). |
| D-28 | java.util.Date or java.time.Instant for date fields? | **java.util.Date** with @JsonFormat(pattern = "yyyy-MM-dd'T'HH:mm:ssXXX") | G-12.2 (follow project patterns) overrides G-01.3 (use Instant). Existing Metadata.java:54-58 uses Date. Known deviation flagged for Reviewer. KD-26. |
